...
public class WeatherReportSendScheduler {
	...
	@Scheduled(cron = "0 0 8 * * *", zone = "Asia/Kolkata")  // Once in a Day at 6 AM => Local(5)
	void sendEmailToGOUsers() throws Exception{  	
		reportService.processUsers(SubscriptionType.GO, 5, 0);
	}
    
	
    @Scheduled(cron = "0 0 8 * * *", zone = "Asia/Kolkata") // Once in a Day at 6 AM => Local(5) + International(5)
	void sendEmailToPROUsers() throws Exception {	 
		reportService.processUsers(SubscriptionType.PRO, 5, 5);
	}
    
	
    @Scheduled(cron = "0 0 8,18 * * *", zone = "Asia/Kolkata") // Twice in a Day at 6 AM & 6 PM => Local(5) + International(5)
